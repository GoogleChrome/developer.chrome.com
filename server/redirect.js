...
// This is used as a nonce to mean "local". It's a fake URL that does not exist
// and is never included in redirect output.
const baseUrlPrefix = 'https://does-not-exist.localhost/';
 ...
 */
function prepareHandler(yamlSource) {
  ...
  return raw => {
    ...
    // If the result URL starts with the matching baseUrlPrefix (with trailing
    // slash), then strip it and treat as local.
...
function build(filename, code = 301) {
  const handler = prepareHandler(fs.readFileSync(filename, 'utf8'));
