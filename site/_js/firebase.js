 ...
 */
 ...
  
 const likeFormat = (num) => new Intl.NumberFormat().format(Math.round(num*10)/10);
  
 const generateLikeCount = (num) => {
   if(num >= 1000000)
     return likeFormat(num/1000000)+'M';
   if(num >= 1000)
     return likeFormat(num/1000)+'k';
   ...
   updateDomNodeCount(year, data) {
     ...
     if (section !== undefined) {
       childrens.forEach((child, idx) => {
         let countLike = generateLikeCount(data[idx].likes);
