...
date: 2022-01-13
updated: 2022-01-13
description: '整页无障碍功能树、在变更标签页中显示更精确的更改以及更多。'
...
_感谢 [阮贝琪 @jecelynyeen](https://github.com/jecelynyeen) 和 [韩昌浩 @hanselfmu](https://github.com/hanselfmu) 提供的翻译_。
...
例如，我在这个[演示页](https://jec.fyi/demo/pup-slow-result)上[录制](/docs/devtools/recorder/#record)用户加载并点击菜单项的操作。但是，菜单项的加载速度很慢（需要 6 秒）。这次用户操作的[重放](/docs/devtools/recorder/#replay)会因此失败，因为它超过了默认的 5 秒超时。
...
我们可以使用新的**超时**设置来解决这个问题。展开我们点击菜单项的步骤。[编辑步骤](/docs/devtools/recorder/#edit-steps)以**添加超时**，并将其设置为**6000**毫秒（等于 6 秒）。
...
通过 **Esc** > **3 点妆** 菜单 > **搜索**打开搜索选项卡。输入搜索字符串（例如函数）并按 **Enter** 以查看搜索结果列表。关注搜索结果并使用以下快捷方式展开/折叠搜索文件：
...
浏览 [Reporting API](https://web.dev/reporting-api/) 文档，以学习如何使用 Reporting API 来监控违规网页，过时的 API 调用以及更多。
