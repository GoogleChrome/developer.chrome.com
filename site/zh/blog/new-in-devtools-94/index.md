...
## DevTools 允许您首选自己喜欢的语言{: #localized }
Chrome DevTools 现在支持超过 80 种语言，允许您首选自己喜欢的语言！

打开[设置](/docs/devtools/customize/#settings)，然后点击**偏好**下的**语言**下拉菜单，选择自己喜欢的语言，并且重新打开 DevTools。

{% Img src="image/dPDCek3EhZgLQPGtEG3y0fTn4v82/eozpCcjmnn7zwya9zXu6.png", alt="在设置页面的首选那里改变语言", width="800", height="494" %}
...
点击 [切换设备工具栏图标](/docs/devtools/device-mode/#viewport) &nbsp; {% Img src="image/admin/9FiBHFCzfPgP8sy6LMx7.png", alt="切换设备工具栏", width="20", height="22" %} &nbsp;，在设备列表那里选择 Nest Hub 或者 Nest Hub Max 设备。
...
## Frame 详情页获取 origin trials 信息{: #origin-trials }
您现在可以从应用面板的 frame 详情页那里获取网站的 [origin trials](/blog/origin-trials/) 信息。

[Origin trials](/blog/origin-trials/) 允许您提前体验新的或者实验特性，以便于在该特性被开放给所有用户之前，开发出一些能够让用户限时体验的功能。

打开带有 origin trials 信息的页面(比如 [demo 页面](https://mediastreamtrack.glitch.me))。在**应用**面板那里，滚动条滚动至 **Frames** 区域，选中 top frame。
...
## 新的 CSS 容器查询徽章 {: #container-queries }
在容器元素（指的是那些匹配 `@container` @规则的祖先元素）的旁边，增加一个新的**容器**徽章。点击该徽章，不但可以切换已选中容器所在浮层的显示状态；而且还可以切换该容器元素所有后代的显示状态。
...
## 新的复选框可反转过滤网络请求的条件 {: #invert-network-filter }
在网络面板那里，使用新的**反转**复选框可反转过滤条件。

比如，您可以输入 “status-code: 404” 来过滤那些状态码为 404 的网络请求。点击**反转**复选框可反转过滤条件（显示那些状态码为不是 404 的网络请求）。
...
## 控制台的边栏接下来会被废弃 {: #deprecated }
为了将过滤界面给移动到工具栏那里，控制台的左边栏将会被移除。如果您有任何担心或者反馈？可以通过这个 [issue tracker](https://crbug.com/1232937) 来让我们知道。

{% Img src="image/dPDCek3EhZgLQPGtEG3y0fTn4v82/CzC2HCaiCcdPgbLykyc8.png", alt="控制台边栏的废弃信息", width="800", height="474" %}
...
DevTools 现在可以在**问题**选项卡那里显示原生 `Set-Cookie` 响应头。

以前，DevTools 不会在网络面板那里显示异常 cookie（不正确的 `Set-Cookie` 响应头）。随着**网络**面板支持新的 `response-header-set-cookie` 过滤条件，用户现在可以过滤掉含有原生 `Set-Cookie` 的响应。DevTools 将会把**问题**选项卡里面的原生 `Set-Cookie` 响应头给链接到**网络**面板。
