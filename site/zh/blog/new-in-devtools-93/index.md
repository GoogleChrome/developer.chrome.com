...
在**元素**面板那里，点击带有 `@container` @规则的 DOM 元素。**样式**窗格现在将显示 `@container` 信息。您可以点击该信息，以编辑容器尺寸。**样式**窗格那里同时也会显示相对应的容器信息。鼠标 hover 上去，可以高亮页面中对应的容器元素以及检查容器的真实尺寸。点击可以选中该容器元素。

容器查询特性现还处于实验性质。测试该特性之前，请在 `chrome://flags` 那里打开 `#enable-container-queries` 开关。
...
[Web bundle](https://web.dev/web-bundles/) 是一种新兴的文件格式，用于封装一个或多个 HTTP 资源。现在，您可以在**网络**面板那里预览 web bundle的网络请求。
...
现在，Attribution Reporting API 错误会显示在**问题**面板那里。
...
在 Chrome 90，**控制台**默认将[输出字符串格式化为 JSON 字面量](/blog/new-in-devtools-90/#double-quotes)。我们从开发者那里得到反馈：这个改动让人有点迷，一些人觉得字符串的转义有点多，不利于字符串的可读性。
...
## 改善 CORS 调试体验 {: #cors }
**控制台**里面的 CORS 相关错误，现在可以链接到**网络** 和**问题**面板。

点击 CORS 相关错误信息旁边的两个新按钮以查看网络请求，以及进一步理清楚该错误信息，在**问题**面板那里获取潜在的解决方案。
