...
*  **Frequency capping** - When the same ad is shown to the user repeatedly, the ad starts to lose its effectiveness. Frequency capping allows you to limit the number of times an ad can be shown to a user. The count of how many times the user has seen the ad is saved in shared storage, and can be accessed [cross-site](https://web.dev/same-site-same-origin/). 
...
* **Known customer** - You might want to render a different element based on whether the user has been seen before at a different site. For example, a payment provider may want to render a "Register" or "Buy now" button based on whether the user has registered before at the payment provider's site. Shared storage can be used to set the status of the user.
* **Reach measurement and custom reporting** - Advertisers can save the user information, such as demographics, reach, frequency measurement, and conversion measurement in shared storage. Then [Private Aggregation API](https://github.com/alexmturner/private-aggregation-api) can be used transmit and processes the data in a privact-preserving way to generate an aggregated report.
