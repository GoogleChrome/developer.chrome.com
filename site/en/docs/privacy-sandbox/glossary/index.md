...
A small piece of textual data on a user's browser that was requested to be
saved by a website. Cookies can be used by a website to save information
associated with a user (or a reference to data stored on the website's
backend servers) as the user moves across the web.
