...
*   An [_aggregatable value_](#aggregatable-value) is an individual data point collected into an aggregation key. If you want to measure how many users from France has seen your content, then `France` is a dimension in the aggregation key, and the `viewCount` of `1` is the aggregatable value.
