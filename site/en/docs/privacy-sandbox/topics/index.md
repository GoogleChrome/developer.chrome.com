...
You can also run the Topics [colab](https://developer.chrome.com/docs/privacy-sandbox/topics/#colab) to try out the Topics [classifier model](https://developer.chrome.com/docs/privacy-sandbox/topics/#classifier-model).

### Taking part in a Topics origin trial

A Privacy Sandbox Relevance and Measurement [origin trial](https://developer.chrome.com/blog/origin-trials/) has been made available in Chrome Beta 101.0.4951.26 and above on desktop for the Topics, [FLEDGE](https://developer.chrome.com/docs/privacy-sandbox/fledge/), and [Attribution Reporting](https://developer.chrome.com/docs/privacy-sandbox/attribution-reporting/) APIs.
...
-   `topic`: a number identifying the topic in the [taxonomy](https://developer.chrome.com/docs/privacy-sandbox/topics/#how-would-topics-be-curated-and-selected)
...
The `chrome://topics-internals` page is available in Chrome on desktop if [you enable the Topics API](https://developer.chrome.com/docs/privacy-sandbox/topics/#feature-flags). This displays topics for the current user, topics inferred for hostnames, and technical information about the API implementation.
...
Users can view information about topics observed for thier browser during the current and previous epochs.

![image](insert_image_url_here)

In this example, recently visited sites included [topics-demo-cats.glitch.me](http://topics-demo-cats.glitch.me/) and [cats-cats-cats-cats.glitch.me](https://developer.chrome.com/docs/privacy-sandbox/topics/cats-cats-cats-cats.glitch.me). This caused the Topics API to select `Pets` and `Cats` as two of the top topics for the current epoch. The remaining three topics have been [chosen at random](https://github.com/patcg-individual-drafts/topics#:~:text=random), since there is not enough browsing history (on sites that observe topics) to provide five topics.
...
Information is provided about the Topics API implementation and settings, such as the [taxonomy](https://developer.chrome.com/docs/privacy-sandbox/topics/#taxonomy) version and [epoch](https://developer.chrome.com/docs/privacy-sandbox/topics/#epoch) duration. These values reflect default settings for the API or parameters successfully set [from the command line](https://developer.chrome.com/docs/privacy-sandbox/topics/#feature-flags). This is handy for checking that command-line flags have worked as expected. In the example below, `time_period_per_epoch` has been set to 15 seconds (the default is seven days).
...
enabled</td>
      <td><br>
Whether the Topics API is enabled.</td>
    </tr>
    <tr>
      <td><p><pre>
...
enabled</td>
      <td><br>
Enables ads APIs: Attribution Reporting, FLEDGE, Topics, Fenced Frames.</td>
    </tr>
    <tr>
      <td><p><pre>
...
disabled</td>
      <td><br>
Enables the third release of the Privacy Sandbox UI settings.</td>
    </tr>
    <tr>
      <td><p><pre>
...
enabled</td>
      <td><br>
If enabled, the browser no longer requires the underlying settings to be enabled for enabling the Privacy Sandbox features.</td>
    </tr>
    <tr>
      <td><p><pre>
...
disabled</td>
      <td><br>
If enabled, the check for whether the IP address is publicly routable will be bypassed when determining the eligibility for a page to be included in topics calculation.</td>
    </tr>
    <tr>
      <td><p><pre>
...
3</td>
      <td><br>
The number of epochs from which to calculate the topics to give to a requesting context. The browser will keep up to N+1 epochs.</td>
    </tr>
    <tr>
      <td><p><pre>
...
7d-0h-0m-0s</td>
      <td><br>
Duration of each <a href="https://developer.chrome.com/docs/privacy-sandbox/topics/#:~:text=epoch">epoch</a>. For debugging, it can be useful to set this to something small like 15 seconds, rather than the default 7 days.</td>
    </tr>
    <tr>
      <td><p><pre>
...
5</td>
      <td><br>
Number of topics calculated per epoch.</td>
    </tr>
    <tr>
      <td><p><pre>
...
5</td>
      <td><br>
Probability that an individual topic within an epoch is one returned at random from the entire <a href="https://developer.chrome.com/docs/privacy-sandbox/topics/#:~:text=taxonomy">taxonomy</a> of topics. That randomness is sticky to an epoch and site.</td>
    </tr>
    <tr>
      <td><p><pre>
...
3</td>
      <td><br>
How many epochs of API usage data (i.e. topics observations) will be used for filtering the topics for a calling context.</td>
    </tr>
    <tr>
      <td><p><pre>
...
1000</td>
      <td><br>
The max number of observed-by context domains to keep for each top topic. The intent is to cap the in-use memory.</td>
    </tr>
    <tr>
      <td><p><pre>
...
100000</td>
      <td><br>
The max number of entries allowed to be retrieved from the database for each query for the API usage contexts. The query will occur once per epoch at topics-calculation time. The intent is to cap the peak memory usage.</td>
    </tr>
    <tr>
      <td><p><pre>
...
30</td>
      <td><br>
The max number of API usage context domains allowed to be stored per page load.</td>
    </tr>
    <tr>
      <td><p><pre>
...
1</td>
      <td><br>
Encodes the Topics API configuration parameters. Each version number should only be mapped to one configuration set. Updating the configuration parameters without updating the `config_version` should be usually fine for local testing, but in some situations could leave the browser in an inconsistent state and/or could let the browser crash, e.g. updating the `number_of_top_topics_per_epoch`.</td>
    </tr>
    <tr>
      <td><p><pre>
...
1</td>
      <td><br>
The <a href="https://developer.chrome.com/docs/privacy-sandbox/topics/#:~:text=taxonomy">taxonomy</a> version used by the API.</td>
    </tr>
  </tbody>
</table>
