...
Now let's try [loading the extension locally][dev-basic-unpacked]. You will see an error dialog box pointing to the problem: 
...
When a manifest key is invalid the extension fails to load, but Chrome gives you a hint of
how to fix the problem. 

Undo that change and enter an invalid permission to see what happens.
...
If the service worker fails to register, you will not be able to access Chrome DevTools until you fix the registration bug. 
...
<figure>
  ...
  <figcaption>
  Refreshing the service worker in the Application panel. (Click to enlarge the image.)
  </figcaption>
</figure>
...
<figure>
  {% Img src="image/BhuKGJaIeLNPW9ehns59NfwqKxF2/M9gIgE8AgIIDYkN2BF58.png", alt="Extensions Management page displaying popup error", width="800", height="588" %}
  <figcaption>
...
<figure>
  {% Img src="image/BhuKGJaIeLNPW9ehns59NfwqKxF2/bnigtWyfrWdtIhNnwQ8r.png", alt="DevTools displaying popup error.", width="800", height="207" %}
  <figcaption>
...
Correct this by calling [`tabs.query()`][tabs-query], then selecting the active tab.
...
you can find logs by inspecting the page or by visiting the Extensions Management page.  
...
[Content scripts][doc-cs] run inside a website. So to find these errors we must inspect the web page the extension is trying to alter:
...
Some extension APIs
require permissions. Refer to the [permissions][doc-perms] article and the [Chrome
