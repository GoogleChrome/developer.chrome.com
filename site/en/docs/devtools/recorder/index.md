...
1. The step above shows two **selectors**. For more information, see [Selector types](#selector-types).
...
For a simple webpage, defining the `id` attribute, CSS `class` attribute is sufficient for the Recorder to detect the selector.

### Common test selectors {: common-test-selector}

However, that might not always be the case. For example, for application developed with modern JavaScript frameworks (e.g. [React](link), [Angular](link), [Vue](link)) and CSS frameworks, the CSS `class` values might be autogenerated. For example this image below.

Take a clearer screenshot https://imgur.com/a/AivwCP2 

There are already some common popular test selectors in the market, and Recorder supports these as well. If you have these common test selectors (show the list) defined, the Recorder will automatically detect and use them first.
...
To create a more resilient test, consider using these test selectors
