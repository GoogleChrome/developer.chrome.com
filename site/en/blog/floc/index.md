...
For other clustering algorithms, a trial might experiment with different inclusion criteria: that's
...
An origin trial for FLoC started in Chrome 89, and has been made available as a [third-party origin trial](https://web.dev/articles/third-party-origin-trials).
...
1.  Start Chrome with the following flags. Make sure to copy all the text!<br> <br>
    ...
    The `FlocPagesWithAdResourcesDefaultIncludedInFlocComputation` flag can also be included, to
    re-create the FLoC algorithm from the initial origin trial (the default value otherwise is
    `false`).
...
+   [Getting started with Chrome's origin trials](/docs/web-platform/origin-trials/): a basic overview.
