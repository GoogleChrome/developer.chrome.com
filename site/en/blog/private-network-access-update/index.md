...
authors:
  - titouan
  - lyf
description: Chrome is deprecating access to local network endpoints from non-secure public websites in Chrome 94 as part of the Local Network Access specification. Read on for recommended actions.
date: 2021-08-26
updated: 2023-01-22
...
Chrome is deprecating access to local network endpoints from non-secure
websites as part of the [Local Network
Access](https://wicg.github.io/local-network-access/) specification, which previously
called Private Network Access. The aim
is to protect users from cross-site request forgery (CSRF) attacks targeting
routers and other devices on local networks. These [attacks have affected
...
* Blocking requests to local networks from insecure public websites starting in
  Chrome 94.
...
* December 2022: Origin trial survey sent, feedback received. Deprecation trial
  decided to be extended to Chrome 113.
* January 2023: The specification is renamed from Private Network Access to Local
  Network Access.
* April 2023: Chrome 113 rolls out to Beta.
* May 2023: Chrome 113 rolls out to Stable. The deprecation trial ends. Chrome
  blocks all local network requests from public, non-secure contexts.

## What is Local Network Access 

[Local Network Access](https://wicg.github.io/local-network-access/)
(formerly known as CORS-RFC1918 or Private Network Access) restricts the ability
of websites to send requests to servers on local networks. It allows such
requests only from secure contexts. The specification also extends the
Cross-Origin Resource Sharing (CORS) protocol so that websites now have to
explicitly request a grant from servers on local networks before being allowed
to send arbitrary requests.

*Local network requests* are requests whose target server's IP address is more
private than that from which the request initiator was fetched. For example, a
request from a public website (`https://example.com`) to a local website
(`http://router.local`), or a request from a local website to localhost.

<figure>
{% Img src="image/YLflGBAPWecgtKJLqCJHSzHqe2J2/nSGfU9xMJxIy6lMjJiMx.png",
alt="Relationship between public, local, localhost networks in Local Network
Access (CORS-RFC1918). ", width="800", height="512" %}
  <figcaption>
    Relationship between public, local, localhost networks in Local Network Access (CORS-RFC1918).
  </figcaption>
</figure>

Learn more at [Feedback wanted: CORS for local networks
...
delivered over HTTPS or from a local IP address) are forbidden from making requests
...
1. Click [**Register**](/origintrials/#/view_trial/4081387162304512001) for the
   Local Network Access from non-secure contexts origin trial to obtain a
...
not implement Local Network Access, so websites might wish to redirect clients
...
If your website needs to issue requests to a target server on a local IP
...
The main problem with serving local websites over HTTPS is that public key
...
3. Inside your local network, configure DNS to resolve `intranet.example` to
   the target server's local IP address. 
4. Configure your local server to use the TLS certificate for
   `intranet.example`. This allows your users to access the local server at
...
expanding the use of end-to-end encryption within your local network.
...
Regardless of Local Network Access, this would likely be a wise investment
anyway.
...
Instead of fetching local subresources from a public web app, a skeleton of
the app can be served from the local server, which then fetches all its
subresources (such as scripts or images) from a public server, such as a CDN.
The resulting web app can then make requests to the local server, as these are
considered same-origin. It can even make requests to other servers with local 
IPs (but not localhost), though this might change in the long term.

By hosting only a skeleton on the local server, you can update the web app by
...
Restricting local network requests to secure contexts is only the first step in
launching Local Network Access. Chrome is working towards implementing the rest of
the specification in the coming months. Stay tuned for updates!

### Restricting localhost access from local websites

The changes in Chrome 94 only affect *public* websites accessing local IP addresses
or localhost. The Local Network Access specification also classifies requests from
*local* websites to localhost as problematic. Chrome will eventually deprecate these
too. This presents a slightly different set of challenges however, as many local 
websites do not have domain names, complicating the use of deprecation trial tokens.

### CORS preflight requests 

The second part of Local Network Access is to gate local network requests
...
Chrome is deprecating and eventually blocking subresource requests to local 
networks. This will not affect navigations to local networks, which can also
be used in [CSRF](https://en.wikipedia.org/wiki/Cross-site_request_forgery)
attacks.

The Local Network Access specification doesn't make a distinction between the
