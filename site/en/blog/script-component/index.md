...
Third-party libraries required for layout should be loaded early to render the page. Third-parties that are not required for initial render should be deferred so that they do not block other processing on the main thread. [Lighthouse](/docs/lighthouse/overview/) has two audits to flag render-blocking or main thread blocking scripts.
