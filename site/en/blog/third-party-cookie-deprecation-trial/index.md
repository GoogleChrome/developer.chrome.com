...
1. If the third-party origin matches a known advertising domain, including if the origin matches an entry on the disconnect.me [advertising list](https://github.com/disconnectme/disconnect-tracking-protection/blob/master/services.json#L1535C11-L1535C11), then the registration request will be rejected. In general, if a domain is considered an advertising domain, subdomains of that domain will also be considered advertising List entries scoped to a particular pathname, however, will have the pathname removed and match to the exact origin in the entry.
...
3. Will sites be able to see which one of their partners have enrolled in the deprecation trial? Will they be able to limit the registration across their partners?
    ...
    * Top level sites won’t be able to limit registration across their partners or the embeds and services on their page. Please contact the partner if that is desired.
