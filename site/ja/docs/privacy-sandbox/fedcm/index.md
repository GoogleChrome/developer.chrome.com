...
{% Aside 'key-term' %}
_ID フェデレーション_は、信頼できる外部パーティ（_ID プロバイダー_ または IdP）に個人（ユーザーまたはエンティティ）の認証または承認を委任します。 すると ID プロバイダーは、その個人のウェブサイト（_リライングパーティ_または RP）へのサインインを許可します。
{% endAside %}

残念ながら、ID フェデレーションの設計基盤（iframe、リダイレクト、Cookie）の仕組みでは、ウェブ全体でユーザーを追跡することも可能です。 ユーザーエージェントは ID フェデレーションと追跡を区別できないため、これらの仕組みが ID フェデレーションをサポートするために使用されているのかどうかを判断するのが困難となっています。

Federated Credential Management API（FedCM）は、ウェブ上のフェデレーション ID フローの抽象化をユースケースごとに抽象化することができます。 この専用の API を用いることで、ブラウザは RP と IdP が情報を交換するコンテキストを理解し、共有されている情報と特権 レベルをユーザーに伝達し、意図しない悪用を防ぐことができます。
...
以下の**すべて**の条件に該当する場合にのみ、FedCM が役立つことを期待しています。
