...
1. `Accept-CH` ヘッダーに `Sec-CH-UA-Reduced を <strong>含めずに</strong>、HTTP レスポンスを送信します。注意: サイトが他の Client Hints をリクエストしない場合、空の値を持つ <code data-md-type="codespan">Accept-CH` は、これを達成するための有効な方法です。
