...
Во вкладке **Производительность** (Performance) очищаются скриншоты и трассировки при клике на кнопку **Начать профилирование и обновить страницу** (Start profiling and reload page).
...
Ранее на вкладке **Производительность** (Performance) отображался таймлайн со скриншотами предыдущей записи. Это затрудняло определение начала нового измерения. Вкладка теперь всегда переходит на страницу `about:blank` чтобы гарантироваться, что записть начнётся с пустой отметки. Это поведение повторяет вкладку **Статистика производительности** (Performance Insights).
...
**Регистратор** (Recorder) теперь предлагает опцию раздельного просмотра кода, что облегчает просмотр кода пользовательского сценария. Чтобы увидеть код пользовательского сценария нажмите кнопку **Показать код** (Show Code).
...
**Регистратор** (Recorder) подсвечивает соответствующий код при наведении курсора на отдельные шаги слева, что облегчает отслуживание пути. Вы можете поменять формат кода в выпадающем меню. Можно переключаться между такими форматами, как скрипты для [Nightwatch Test](https://bit.ly/nightwatch-recorder). 
...
Вы можете создавать записи, в которых отслеживаются только нужные вам типы селекторов. С новой опцией настройки типов селекторов вы можете выбрать или исключить из записи селекторы типа XPath, что обеспечит запись только важных для вас селекторов.
...
- При нажатии `Enter` выполняется напечатанная строка. Ранее нажатие на `Enter` приводило к подстановке первого предложения из списка автодополнения. Теперь для выбора одного из вариантов автодополнения нажмите `Tab` или `Стрелку вправо`.
...
- Устранена проблема регрессии в DevTools, когда он не останавливался на операторе `debugger` во встроенных скриптах. ([1385374](https://crbug.com/1385374))
...
- Панель [Фрагменты (Snippets)](/docs/devtools/javascript/snippets/) во вкладке **Источники** (Sources) теперь поддерживает автодополнение, по примеру **Консоли** (Console). ([772949](https://crbug.com/772949))
