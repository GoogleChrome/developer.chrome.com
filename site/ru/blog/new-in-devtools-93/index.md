...
`#enable-container-queries` по адресу `chrome://flags`, чтобы включить эту функциональность.
...
это новое API, которое поможет без всяких межсайтовых идентификаторов понять,
...
Начиная с версии 90, во вкладке **Console** Chrome DevTools [строки
всегда форматируются как валидные
JSON-литералы](/blog/new-in-devtools-90/#double-quotes). Мы получили отзывы от
разработчиков, что это изменение слегка сбивает с толку. Некоторые посчитали,
что экранирование слишком громоздкое и затрудняет чтение вывода. 

Во вкладке **Console** строки теперь форматируются как валидные
JavaScript-литералы, а вам предоставляется выбор из трёх опций копирования строки.
...
запрос или прочитать расширенное сообщение об ошибке и варианты решения
на вкладке Issues. 
...
оно может быть выбрано в выпадающем меню при клике по «Создать
...
## Исправление перечисления CSS-селекторов {: #matching-selectors }

Исправлено отображение перечисления CSS-селекторов в DevTools. Оно работало
неправильно в прошлой версии.

Селекторы, разделённые запятой, в панели **Styles** окрашиваются в разные цвета
в зависимости от соответствия выбранному узлу DOM:
