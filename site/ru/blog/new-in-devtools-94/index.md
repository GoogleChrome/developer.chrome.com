...
## Поддержка новых устройств Nest Hub {: #nest-hub }

Теперь вы можете симулировать разрешения экранов устройств Nest Hub и Nest Hub Max в [соответствующем режиме](/docs/devtools/device-mode/).
...
## Информация про origin trials сайта при просмотре свойств фрейма {: #origin-trials }

Вы можете получить информацию об [origin trials](/blog/origin-trials/) сайта в панели Frames вкладки Application.

[Origin trials](/blog/origin-trials/) дают вам доступ к новым или экспериментальным возможностям Chrome для добавления функциональности, которую ваши пользователи могут опробовать раньше, чем эти возможности станут доступны для всех.

Откройте страницу с origin trials (например, [демо-страницу](https://mediastreamtrack.glitch.me)). В панели **Application** прокрутите вниз до секции **Frames** и выберите верхний фрейм.
...
Новый значок **container** появляется рядом с элементов-контейнером (родительский элемент, соответствующий условию директивы `@container`). Кликните на значок, чтобы увидеть наложение выбранного контейнера и всех его потомков на странице.
...
Например, вы можете написать "status-code: 404", чтобы показать только сетевые запросы со статусом 404. Включите чекбокс **Invert**,  и фильтр  инвертируется — в результатах будут показаны только сетевые запросы со статусом, отличным от 404.
...
## Предстоящее удаление боковой колонки в панели Console {: #deprecated }

Боковая колонка в панели Console будет убрана, а фильтры переедут в панель инструментов. Есть возражение или мнение по этому вопросу? Выскажитесь в [трекере задач](https://crbug.com/1232937).
...
## Отображение необработанных заголовков `Set-Cookie` во вкладке Issues и в панели Network {: #raw-cookies }
...
Теперь вы можете изменить формат цвета у любого элемента прямо из вкладки Computed, удерживая<kbd>Shift</kbd> и кликнув на превью цвета.
...
## Собственные всплывающие подсказки заменены на нативные HTML-подсказки {: #tooltip }

DevTools теперь поддерживает нативные HTML-подсказки во всех компонентах. В DevTools долгое время использовалась собственная реализация всплывающих подсказок, из-за невозможности стилизовать стандартные.

Поддержка собственной реализации всплывающих подсказок была достаточно сложна, и мы регулярно сталкивались с нетривиальными проблемами.

Взвесив все преимущества и недостатки, мы пришли к выводу, что для DevTools достаточно нативных HTML-подсказок. Их использование позволяет избежать множества проблем для наших пользователей.
...
Во вкладке **Issues** наведите курсор на задачу, кликните на иконку меню  &nbsp; {% Img src="image/admin/4sdCQbpBaG4MpoHB1J08.png", alt="More", width="4", height="20" %} &nbsp; справа и выберите **Hide issues like this**, чтобы скрыть её.
