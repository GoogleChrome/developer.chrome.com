...
В панели **Элементы** (Elements) откройте панель **Специальные возможности** (Accessibility) и поставьте галочку в чекбоксе **Дерево специальных возможностей на всю страницу** (Full-page accessibility tree). После этого перезагрузите DevTools и вы увидите новую кнопку в панели **Элементы** (Elements).
...
Нажмите на кнопку чтобы вкоючить режим **Дерево специальных возможностей на всю страницу**. Вы можете развернуть отдельные узлы или посмотреть подробноести в панели **Специальные возможности** (Accessibility).
...
Раньше дерево доступности было доступно только во вкладке **Специальные возможности** (Accessibility). Область обзора была ограниченной, можно было исследовать только один элемент и его потомков.
...
Наша команда все еще активно работает над этой новой функцией, и мы ждем [ваши отзывы](https://goo.gle/devtools-a11y-tree-feedback) для дальнейших улучшений.
...
## Установливайте более длительный тайм-аут для записи поведения пользователя {: #recorder-timeout }
...
Теперь вы можете настроить параметр **Timeout** во вкладке [Recorder](/docs/devtools/recorder/) для всех шагов или конкретного шага. Это полезно, особенно для страниц с медленными сетевыми запросами и длинной анимацией.
...
Мы можем использовать новую настройку **Timeout** чтобы исправить это. Расширьте шаг ожидания нажатия на пункт меню. [При редактировании шага](/docs/devtools/recorder/#edit-steps) нажмите кнопку **Add timeout** и установите значение **6000** миллисекунд (что равно 6 секундам).
...
По желанию вы можете настроить **Timeout** для всех шагов в меню **Replay settings**. Раскройте настройки **Replay settings** и измените значение в окне **Timeout**. 
...
Новая вкладка **Возвратный кэш** (Back/forward cache) поможет протестировать ваши страницы, чтобы убедиться, что они оптимизированы для возвратного кэша или выявить проблемы, которые можно исправить.
...
## Новый фильтр во вкладке Свойства (Properties) {: #properties }
...
Если вы хотите сфокусироваться на конкретном свойстве во вкладке **Совйства** (Properties), то вы можете ввести имя нужного свойства или его значение в новом текстовом поле **Фильтр** (Filter).
...
Эти нововведения позволят вам быстрее добраться до объектов, которые вам интересны, и тем самым повысить вашу производительность!
...
Медиафункция [forced-colors](https://drafts.csswg.org/mediaqueries-5/#forced-colors) в CSS используется для определения активного режим принудительного выбора цветов в браузере пользователя (например, режим высокой контрастности в Windows). В этом режиме к странице применяется ограниченная пользовательская палитра.
...
Откройте пункт [Выполнить команду](/docs/devtools/command-menu/), выполните команду **Показать "Отрисовка"** (Show Rendering) и затем установите нужное значение в выпадающем меню **Эмулировать медиафункцию CSS orced-colors** (Emulate CSS media feature forced-colors).
...
### Новые сочетания клавиш для повторения запроса XHR в панели Сеть (Network) {: #replay-xhr }
...
Выберите XHR-запрос в панели **Сеть** (Network) и нажмите **R** на клавиатуре чтобы повторить запрос XHR. Ранее вы могли воспроизвести XHR только через контекстное меню (правый клик мыши > **Повторить запрос XHR**)
...
### Горячие клавиши для расширение всех результатов поиска {: #toggle-search-result }
...
Перейдите на страницу с [сочетаниями клавиш](/docs/devtools/shortcuts/) чтообы получить полную информацию о сочетаниях клавиш в Chrome DevTools.
...
Во вкладке **Lighthouse** теперь запускается Lighthouse 9. Lighthouse теперь будут перечисляться все элементы, имеющие одинаковый идентификатор.
...
## Улучшена вкладка Источники (Sources) {: #sources }
...
Улучшена стабильность панели **Источники** за счёт перевода на [CodeMirror 6](https://codemirror.net/6/). Вот несколько заметных улучшений:
...
- Больше никакой случайной прокрутки при переходе по коду
...
Вот некоторые заслуживающие внимания исправления в этом выпуске:
...
- Выделение кода ломалось при поиске в документах с очень длинными строками во вкладке **Источники** (Sources). Теперь это исправлено. ([1275496](https://crbug.com/1275496))
...
Экспериментальная панель **Reporting API** была представлена в [Chrome 96](/blog/new-in-devtools-96/#reporting-api), чтобы помочь вам отслеживать созданные на странице отчеты и их состояние.
...
Изучите информацию об использовании [Reporting API](https://web.dev/reporting-api/) для отслеживания проблем безопасности, устаревших вызовов API и т.д.
