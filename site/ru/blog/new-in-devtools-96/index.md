...
Используйте новую вкладку **Обзор CSS** (CSS Overview) чтобы увидеть советы по улучшению CSS на вашем сайте.
...
Вы можете пойти дальше. Например, кликните на цвет в секции **Цвета** чтобы увидеть список всех элементов, к которым применяется выбранный цвет. Кликните на элемент чтобы увидеть его на вкладке **Элементы** (Elements).
...
Вкладка **Обзор CSS** (CSS Overview) пока в раннем доступе. Наша команда прямо сейчас активно работатет над ней и и нам нужна [обратная связь](https://goo.gle/css-overview-feedback) от вас для дальнейших улучшений.
...
## Улучшение вкладки Отрисовка

<!-- ### Emulate the CSS prefers-contrast media feature {: #prefers-contrast } -->
### Эмуляция CSS-медиафичи prefers-contrast {: #prefers-contrast }

{% Img src="image/dPDCek3EhZgLQPGtEG3y0fTn4v82/47fsHvVLiVC9J0eWY9wD.png", alt="Эмуляция CSS-медиафичи prefers-contrast", width="800", height="483" %}
...
Медиафича [prefers-contrast](https://www.chromestatus.com/feature/5646323212615680) используется для определения того, запросил ли пользователь больший или меньший кнотраст на странице.
...
### Эмуляция функции создания автоматической тёмной темы в Chrome {: #auto-dark-mode }
...
В Chrome 96 анонсирован запуск [тестирования](/blog/origin-trials/) функции [Auto Dark Theme](/blog/auto-dark-theme/) на Android. Эта функция позволяет браузеру применить автоматически сгенерированную тёмную тему на сайтах со светлой цветовой схемой когда пользователь включает тёмную тему в операционной системе.
...
Откройте [Командное меню (Command Menu)](/docs/devtools/command-menu/), выполните команду **Показать "Отрисовка"** (Show Rendering) и выберите нужное значение в выпадающем меню **Эмулировать автоматическую тёмную тему** (Emulate auto dark mode).
...
## Копирование объявлений как JavaScript в панели Стилей {: #copy-as-js }
...
Две новые опции были добавлены в контекстное меню для облегчения копирования CSS-правил как JavaScript-свойств. Эти опции будут особенно удобны для разработчиков, работающих с [CSS-in-JS](/blog/css-in-js/#what-is-css-in-js) библиотеками.
...
Во вкладке **Стили** (Styles) кликните правой кнопкой мыши на CSS-правиле. Можно выбрать **Копировать объявление как JS** (Copy declaration as JS) чтобы скопировать одно правило или **Копировать все объявления как JS** (Copy all declarations as JS) чтобы скопировать все правила.

<!-- For instance, the example below will copy `padding-left: '1.5rem'` to the clipboard. -->
В примере ниже в буфер обмена будет скопировано `padding-left: '1.5rem'`.
...
## Новая панель Полезная нагрузка на вкладке Сеть {: #payload }
...
Используйте новую панель **Полезная нагрузка** (Payload) на вкладке **Сеть** (Network) panel при проверке сетевого запроса с полезной нагрузкой. Ранее информация о полезной нагрузке была доступна на вкладке **Заголовки** (Headers).
...
На вкладке **Свойства** (Properties) теперь показываются только соответствующие свойства вместо всех свойств экземпляра.
...
Балогодаря этому изменению и [улучшению](/blog/new-in-devtools-95/#properties) вкладки **Свойства** (Properties) в Chrome 95 вы сможете легче находить соответствующие свойства.
...
Вы можете скрыть ошибки CORS в **Консоли** (Console). Поскольку ошибки CORS теперь показываются в панели Проблемы их скрытие может в панели **Консоль** (Console) может помочь уменьшить беспорядок.
...
Объекты [Intl](https://tc39.es/ecma402/#intl-object) теперь имеют удобный предварительный просмотр и их легко оценить в Консоли (Console). Ранее объекты `Intl` не отображались надлежащим образом.
...
### Последовательный стек трассировки async {: #async }
...
Боковую колонку в Консоли (Console) решено оставить. В Chrome 94 мы анонсировали [упразднение боковой колонки в Консоли](/blog/new-in-devtools-94/#deprecated) и попросили разработчиков высказать свои размышления и сомнения по этому поводу.
...
Панель [Кэш приложения](https://developer.chrome.com/docs/devtools/storage/applicationcache/) на вкладке Приложение удалена поскольку [AppCache](https://web.dev/appcache-removal/) больше не поддерживается в Chrome и других браузеров на основе Chromium.
...
После включения этого эксперимента вы сможете просматривать отчёты в панели **Reporting API** во вкладке **Приложение** (Application). 
