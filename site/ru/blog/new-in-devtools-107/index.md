...
Теперь вы можете кастомизировать горячие клавиши для ваших любимых комманд в DevTools.
...
## Переключение между светлой и тёмной темами по сочитанию клавиш {: #toggle-themes }
...
{% Img src="image/dPDCek3EhZgLQPGtEG3y0fTn4v82/7oGdE2eRsgwokWXW9XvA.png", alt="Переключение между светлой и тёмной темами по сочитанию клавиш", width="800", height="576" %}
...
Распознавание байтов объекта в окружении памяти WebAssembly — болезненная вещь. Вам нужно знать размер объекта и считать байты от его начала.
...
Эта функция поможет вам отличить их от окружающей памяти. Читайте [Extending the Memory Inspector for C/C++ debugging](/blog/memory-inspector-extended-cpp/) чтобы узнать о других измнениях.
...
Полная информация о **Инициаторе** (Initiator) теперь доступна для [импорта HAR](/docs/devtools/network/reference/#save-as-har). Ранее во вкладке **Сеть** (Network) показывалась только частичная информация о инициаторе при импорте.
...
Для пользователей, особенно инженеров тестирования, которые всегда работают с длинными поисковыми запросами, такое поведение далеко от идеала. Дерево DOM может прыгать множество раз пока вы набираете длинный поисковый запрос (например, `//div[@id="example"]`). Такое поведение создает ненужные скачки.
