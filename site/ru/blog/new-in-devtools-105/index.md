...
Теперь вы можете настроить точки останова и пошагово воспроизвести пользовательские сценарии во вкладке **Recorder**.
...
Чтобы установить точку останова кликните на синюю точку рядом с шагом. Воспроизведите пользвоательский сценарий, проигрывание остановится перед отмеченным шагом. С этого места вы можете продолжить воспроизведение, выполнить только этот шаг или отменить воспроизведение. 

<!-- With this feature, you can fully visualize and debug your user flow with ease. -->
С помощью этой функции вы можете полностью визуализировать и с легкостью отладить пользовательский сценарий.
...
Во вкладке **Recorder** теперь можно вручную добавить шаг с событием mouse over (hover) в запись. 

<!-- [This demo](https://jec.fyi/demo/menu-hover) shows a pop up menu on hover. Try to record a user flow and click a menu item. -->
В [демо](https://jec.fyi/demo/menu-hover) по наведению курсора на меню. Попробуйте записать пользовательский сценарий и во время записи кликните на пункт меню.
...
Если вы воспроизведёте запись, то она проиграется с ошибкой, потому что сейчас **Recorder** во время записи не ловит событие наведения мыши автоматически. Чтобы решить эту проблему [вручную добавьте шаг](/docs/devtools/recorder/reference/#add-and-remove-steps) чтобы имитировать событие ховера перед кликом на пункт меню.
...
Читайте документацию [Performance Insights](/docs/devtools/performance-insights/) чтобы узнать, как получить полезные сведения и улучшить производительность вашего сайта с помощью панели. 
...
Во вкладке **Performance Insights** теперь будет фиксироваться [вспышки невидимого текста (FOIT) и вспышки нестилизованного текста (FOUT)](https://web.dev/preload-optional-fonts/#font-rendering) как потенциальные причины сдвига раскладки.

<!-- To view the potential root causes of a layout shift, click on a screenshot in the **Layout shifts** track. -->
Чтобы увидеть потенциальные причины сдвига раскладки кликните на скриншот в треке **Layout shifts**.
...
## Обработчик протоколов в панели Манифест {: #manifest }
...
Регистрация обработчика протокола URL позволяет установленным PWA обрабатывать ссылки с определённым протоколом (например [`magnet`](https://wikipedia.org/wiki/Magnet_URI_scheme), `web+example`) для более полноценного опыта использования. 
...
Перейдите в раздел **Обработчики протоколов** (Protocol Handlers) во вкладке **Приложение** (Application) > **Манифест** (Manifest). Вы можете увидеть и протестировать здесь все доступные протоколы.
...
Например, установите это [тестовое PWA](https://protocol-handler.glitch.me/). В разделе **Обработчики протоколов** (Protocol Handlers) наберите «americano» и нажмите **Тестировать протокол** (Test protocol) чтобы открыть страницу с кофе в PWA.
...
Элемент [`<dialog>`](https://web.dev/building-a-dialog-component/) недавно получил стабильную поддержку во всех браузерах. Когда вы открываете диалог он помещается в [верхний слой](/blog/top-layer-devtools/). Контент верхнего уровня отображается поверх всего остального контента. 
...
Чтобы помочь визуализировать элементы верхнего слоя, в DevTools добавляет контейнер верхнего слоя (`#top-layer`) в DOM-дерево. Он размещается после закрывающего тега `</html>`.
...
Откройте файл Wasm во вкладке **Источники** (Sources). Кликните правой клавишей мыши в области редактора и выберите **Добавить сведения для отладки DWARF…** (Add DWARF debugging info…) чтобы прикрепить отладочную информацию.
...
Теперь вы можете редактировать самую верхнюю функцию в стеке без необходимости перезапускать отладчик.
...
В Chrome 104 в DevTools вернулась возможность [перезапуска фрейма](/blog/new-in-devtools-104/). Тем не менее вы не могли редактировать функцию, на которой вы остановились прямо сейчас. Обычно разработчики прерывают функцию, а затем редактируют ее во время паузы.

<!-- With this update, the debugger automatically restarts the function with the following restrictions: -->
С этим обновлением отладчик автоматически перезапускает функцию при следующих ограничениями:

<!-- - Only the top-most function can be edited while paused -->
<!-- - No recursive call on the same function further down the stack -->
- Только функция верхнего уровня может быть отредактирована во время паузы.
- Ниже по стеку нет рекурсивных вызовов одной и той же функции.
...
Теперь вы можете видеть и редактировать [CSS-директиву `@scope`](https://drafts.csswg.org/css-cascade-6/#scope-atrule) во вкладке **Стили** (Styles).
...
Откройте эту [демо-страницу](https://codepen.io/miriamsuzanne/details/ZErXZVY) и проинспектируйте гиперссылку внутри элемента `<div class="dark-theme">`. Во вкладке **Стили** (Styles) найдите директиву `@scope`. Кликните на объявление правила чтобы отредактировать его.
...
Поддержка `@scope` находится в стадии разработки. Чтобы протестировать эту функцию включите флаг **Experimental Web Platform features** на странице `chrome://flags/#enable-experimental-web-platform-features`.
{% endAside %}
...
- Правильная реализация удаления пары ключ-значение локального хранилища из таблицы во вкладке **Приложении** (Application) > **Локальное хранилище** (Local Storage) при её удалении. ([1339280](https://crbug.com/1339280))
...
- Предпросмотр цвета теперь отображается корректно при просмотреть файлов CSS во вкладке **Источники** (Sources). Ранее его позиция была неверной. ([1340062](https://crbug.com/1340062))
