...
하지만 뷰포트와 유저 에이전트를 사용한 쿼리 방식 대신, `@container` 쿼리는 특정한 조건에 일치하는 조상 컨테이너를 사용합니다.

**Elements** 패널에서 `@container` 규칙을 가지고 있는 DOM 요소를 선택하면 이제 DevTools 에서 `Styles` 패널에서 `@container` 정보를 보여줍니다.
요소를 클릭하고 사이즈를 변경해보세요. **Styles** 패널에서 컨테이너에 상응하는 정보를 보여줄 것입니다.
페이지에서 컨테이너 요소를 강조 표시하고 컨테이너 크기를 확인하려면 마우스를 가져갑니다.
컨테이너 요소를 확인하려면 해당 요소를 클릭하세요.

컨테이너 쿼리는 현재 실험 기능입니다. `chrome://flags` 에서 `#enable-container-queries` 플래그를 활성화한 상태로 테스트하세요.

{% Img src="image/dPDCek3EhZgLQPGtEG3y0fTn4v82/3NzGBpukHQfUZUKUpUgf.png", alt="Styles 패널에서 CSS 컨테이너 쿼리를 수정할 수 있습니다.", width="800", height="554" %}
...
[Web bundle](https://web.dev/web-bundles/) 은 단일 파일에서 하나 이상의 HTTP 리소스를 캡슐화한 파일 포맷입니다. 이제 **Network** 패널에서 web bundle 콘텐츠를 미리볼 수 있습니다.
...
Chrome 90에서 DevTools는 **Console** 에서 [문자열 출력을 언제나 유효한 JSON 리터럴로 변경](https://developer.chrome.com/en/blog/new-in-devtools-90/#double-quotes) 하도록 업데이트 하였습니다. 이러한 변경사항이 오히려 혼란을 줄 수 있다는 피드백을 개발자로부터 받았고, 일부는 이스케이프 양이 과도하거나, 출력된 값을 읽기 어렵다고 느끼기도 하였습니다.
...
이제 **Lighthouse** 패널은 Lighthouse 8.1로 동작합니다.
...
리포트에는 새로운 메트릭 필터 (스크린샷에서 **Show audits relevant to** 필터 참고)를 포함하고 있습니다. 특정 메트릭을 선택하여 해당 메트릭을 개선하는 데 가장 관련성이 높은 진단 결과 및 개선 사항에 초점을 맞춥니다.
...
{% Img src="image/dPDCek3EhZgLQPGtEG3y0fTn4v82/2Cwggroar7pNesfAQi4K.png", alt="Manifest 패널의 new note URL", width="800", height="477" %}
...
**Styles** 패널에서 쉼표로 구분된 셀렉터는 선택된 DOM 노드와의 매칭 여부에 따라 색상이 다르게 지정됩니다.
...
{% Img src="image/dPDCek3EhZgLQPGtEG3y0fTn4v82/x2NKXwJPzjycjeD7cLH6.png", alt=" Network 패널에서 JSON 응답 Pretty-printing", width="800", height="523" %}
